...
bot_status = {
    "bot": "GOLD V4.4 WIDE STOP BUDAPEST FIX",
    "fut": True,
    "allapot": "Fut - V4.4 Budapest +2h WIDE",
    "utolso_ellenorzes": "meg nem futott",
    "utolso_jel": "meg nincs jel",
    "mai_jelek": 0,
    "toke_vedelem": f"ATR x{ATR_MULT} PRO (max {MAX_TOKE_BUKO}% | {LEVERAGE}x | MIN {MIN_ATR}$ + {WICK_BUFFER}$ buf)",
...
@app.route("/ping")
def ping():
    return jsonify({"status": "alive", "time": now_budapest().isoformat(), "bot": "GOLD V4.4 WIDE STOP"})
...
def main_loop():
    ...
    print(f"GOLD V4.4 WIDE STOP BUDAPEST FIX indul - ATR x{ATR_MULT} MIN {MIN_ATR}$")
    send_telegram(f"GOLD V4.4 WIDE STOP BUDAPEST FIX elindult EMA {EMA_FAST}/{EMA_MID}/{EMA_SLOW} ATR x{ATR_MULT} MIN {MIN_ATR}$ max {MAX_TOKE_BUKO}% - STOP/TP figyelo AKTIV")
    while True:
        try:
            res = analyze()
            if not res:
            ...
            if res["jel"]:
                ...
                if res["jel"] == utolso_jel_tipus and (most - utolso_jel_ido) < 1800:
                    ...
                    uzenet = f"{res['jel']} JEL - XAUUSD V4.4 WIDE STOP Ar: {res['ar']:.2f} ATR: {res['atr_used']:.2f} x{ATR_MULT} = {res['stop_dist']:.2f}$ RSI: {res['rsi']:.1f} Stop: {stop_ar:.2f} ({toke_pct:.1f}% toke) TP: {tp_ar:.2f} (+{toke_pct*2:.1f}% RR 1:2) STOP/TP jelzest kuldok! {LEVERAGE}x LEV = RISK"
                    send_telegram(uzenet)
