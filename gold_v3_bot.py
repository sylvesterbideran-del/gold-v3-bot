...
def analyze():
    ...
    # ATR stop calc V4.1 - MIN 3$ FIX (te kérdezted: 3% vs 5%)
    raw_atr = float(last["atr"]) if pd.notna(last["atr"]) else 8.0
    # MINIMUM ATR = 3$ -> ha PAXG proxy 0.10$-t ad hétvégén, ne legyen 0.16$ stop!
    MIN_ATR = 3.0
    ...
    bot_status["toke_vedelem"] = f"ATR x{ATR_MULT} = {stop_dist_price:.2f}$ ({toke_buko_atr:.1f}% tőke | {LEVERAGE}x | MIN 3$)"
