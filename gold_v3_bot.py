...
bot_status = {
    "bot": "GOLD V4.3.2 - XAUUSD ATR PRO + STOP/TP ALERT + BUDAPEST FIX",
    "fut": True,
    "allapot": "✅ Fut - V4.3.2 STOP/TP figyelő aktív (Budapest +2h)",
...
def home():
    try:
        ...
        return jsonify({"uzenet": "Gold V4.3.2 ATR PRO STOP/TP BUDAPEST fut", "status_link": "/status", "hiba": str(e)})
...
@app.route("/ping")
def ping():
    return jsonify({"status": "alive", "time": now_budapest().isoformat(), "bot": "GOLD V4.3.2 ATR PRO STOP/TP BUDAPEST"})
...
def main_loop():
    ...
    print(f"GOLD V4.3.2 ATR PRO STOP/TP BUDAPEST FIX indul - ATR x{ATR_MULT} MIN {MIN_ATR}$")
    send_telegram(f"GOLD V4.3.2 ATR PRO STOP/TP BUDAPEST FIX elindult EMA {EMA_FAST}/{EMA_MID}/{EMA_SLOW} ATR x{ATR_MULT} MIN {MIN_ATR}$ max {MAX_TOKE_BUKO}% - STOP/TP figyelő AKTÍV")
    while True:
        try:
            res = analyze()
            if not res:
